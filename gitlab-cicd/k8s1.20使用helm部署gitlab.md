...
```
http://gitlab.apps164103.hisun.local/
```
...
点项目左边菜单中 运维--> Kubernetes --> 与集群证书集成——> 连接现有集群-->
...
查看 default-token-xxxxx 的 secret token
...
访问 `http://sonar.apps164103.hisun.local/`
...
```
SONAR_HOST_URL 	http://sonar.apps164103.hisun.local
...
variables:
  # Gitlab Project Repo
  REPO_NAME: gitlab.apps164103.hisun.local/liujinye/gin-helloworld
  # goproxy
...
```
http://gitlab.apps164103.hisun.local/admin/application_settings/network
``` 
